...
def marginalize(factor, var):
    ...
    for assignment in itertools.product([True, False], repeat=len(vars_new)):
        ...
        for val in [True, False]:
            ...
            full[var] = val
            prob += factor.table.get(
                tuple(full[v] for v in factor.variables), 0
            )
...
def restrict(factor, var, val):
    ...
    for assignment in itertools.product([True, False], repeat=len(vars_new)):
        full = dict(zip(vars_new, assignment))
        full[var] = val
        table_new[assignment] = factor.table.get(
            tuple(full[v] for v in factor.variables), 0
        )
...
def variable_elimination(factors, query, evidence, hidden):
    for ev, val in evidence.items():
        factors = [
            restrict(f, ev, val) if ev in f.variables else f
            for f in factors
        ]
...
print(
    "P(B | J=1, M=1):",
    variable_elimination(factors, "B", evidence, hidden)
)import itertools
